...
// TMDB's genre id -> name lists. These are a stable, documented vocabulary
// (https://developer.themoviedb.org/reference/genre-movie-list /
// genre-tv-list) so this is hardcoded rather than fetched — every list
// endpoint we call already returns `genre_ids` inline, so mapping them to
// names for the on-image tag row costs zero extra TMDB requests.
...
// Up to 3 genre names, in TMDB's own (relevance) order, for the on-image tag
// row. Bloat genres (talk/news/reality/soap/TV-movie) never reach here since
// isBloat() already drops those items earlier in the pipeline.
function genreNamesFor(genreIds, mediaType) {
  if (!Array.isArray(genreIds)) return [];
  const table = mediaType === 'tv' ? TV_GENRE_NAMES : MOVIE_GENRE_NAMES;
  const names = [];
...
async function fetchPoolFresh(pool) {
  ...
  if (pool === 'trending') {
    ...
    // Rank within this week's trending list (1-indexed, TMDB's own order) —
    // reused as-is for the "Trending" tag rather than fetching a separate
    // trending/day list, since trending/week is already fetched for the pool
    // freshness merge above. Zero extra TMDB calls; the tradeoff is the tag
    // reads "trending this week," not "trending today."
    const trendingMovieRank = new Map(trendingMovies.results.map((m, i) => [m.id, i + 1]));
    const trendingTvRank = new Map(trendingTv.results.map((t, i) => [t.id, i + 1]));
    ...
    items = [
      ...
      ...it,
      trendRank: it.media_type === 'movie' ? trendingMovieRank.get(it.id) : trendingTvRank.get(it.id),
    }));
  ...
  return deduped.map((it) => ({
    ...
    genreNames: genreNamesFor(it.genre_ids, it.media_type),
    trendRank: it.trendRank ? { rank: it.trendRank, mediaType: it.media_type } : null,
  }));
}
