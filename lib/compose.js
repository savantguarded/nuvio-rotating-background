...
// Sentence case per Charles's ask (replacing the earlier all-caps styling):
// lowercase the whole string, then capitalize just its first letter.
 ...
 * Sentence case (not the original all-caps) is also Charles's ask, applied
 * via toSentenceCase() above. Either line can be omitted independently (an
 * item may have genres but no trend rank, or vice versa in principle).
 */
function buildTagsSvg({
  ...
  if (genreNames.length) {
    const text = toSentenceCase(genreNames.join('   ·   '));
    elements.push(
  ...
  if (trendRank) {
    const text = toSentenceCase(`No. ${trendRank.rank} trending today`);
    elements.push(
      `<text x="${rightX}" y="${y}" text-anchor="end" font-family="Arial, Helvetica, sans-serif" ` +
